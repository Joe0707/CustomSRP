...
{
    float horizontal = 2 * abs(luma.n + luma.s - 2.0 * luma.m) + abs(luma.ne + luma.se - 2.0 * luma.e) + abs(
        luma.nw + luma.sw - 2 * luma.w);
    float vertical = 2 * abs(luma.e + luma.w - 2.0 * luma.m) + abs(luma.ne + luma.nw - 2.0 * luma.n) + abs(
        luma.se + luma.sw - 2.0 * luma.s);
